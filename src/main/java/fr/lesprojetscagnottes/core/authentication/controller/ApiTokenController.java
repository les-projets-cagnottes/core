...
public class ApiTokenController {
    ...
    public List<AuthenticationResponseModel> list(Principal principal) {
        ...
        List<AuthenticationResponseEntity> apiTokens = apiTokenRepository.findAllByUserId(user.getId());
        List<AuthenticationResponseModel> authenticationResponses = new ArrayList<>();
        apiTokens.forEach(apiToken -> {
            apiToken.setToken(StringsCommon.EMPTY_STRING);
    ...
    public void delete(Principal principal, @PathVariable("id") long id) {
        UserEntity user = userService.get(principal);
        AuthenticationResponseEntity apiToken = apiTokenRepository.findByIdAndUserId(id, user.getId());
        if(apiToken == null) {
